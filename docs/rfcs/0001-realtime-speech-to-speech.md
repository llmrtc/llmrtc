...
from end-of-speech to first audio. Native speech-to-speech models
...
2. **Upstream audio**: `peer.on('audioData')` frames (48kHz Int16) are
   resampled to `session.inputSampleRate` by the existing
   `AudioProcessor` tee (Phase 4 infrastructure, VAD not required —
   the tee runs in pass-through mode streaming *all* frames, since turn
   detection is provider-side). Frames go straight to
   `session.sendAudio()`.
...
- Web client: new events `assistantTranscript(text, isFinal)` and
  `usage(usage)`; unknown-message handling in current clients ignores
  unrecognized types, so old clients degrade gracefully (verified
  behavior of the `default:` branch in the client message switch).
...
3. **Session budget**: `realtimeSpeech.budget = { maxSessionMs?,
   maxTokens?, onExceeded: 'warn' | 'end-session' }` enforced from
